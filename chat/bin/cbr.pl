...
$string =~ /\s(.*?)\s(.*)$/;
	...
	my $response = $ua->get('http://www.cbr.ru/scripts/XML_daily.asp');             
    my $date;                                                                       
	my $rub;                                                                        
		                                                                                   
	if ($response->is_success) {                                                    
	my $string = $response->decoded_content;                                
	$string =~ /<ValCurs Date="(.*)" name="Foreign.*$valuta.*?<Value>(.*?)</s;                                                                              
	$date = $1;                                                             
    $rub = $2;                                                              
    $rub =~ tr/,/./;                                                        
    }                                                                       
		...
		return "$cost $valuta $date = " . $cost * $rub . " RUB";
	}
...
my $server = Local::Chat::ServerConnection->new(nick => $nick, host => $ARGV[0] || 'localhost', 
	...
	on_message => sub {
		...
		if ($message->{text} && $message->{text} =~ m/^!who$/){
			$srv->message({text=>"i am cbr"})
		}

		if ($message->{text} && $message->{text} =~ m/^cbr/){
			$message->{text} =~ /^cbr(.*)$/;
			$srv->message({text=>$message->{from}.calc($1)});
